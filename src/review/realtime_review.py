...
class _SpecReviewOutcome:
    ...
    dicts — one per API call actually made (plan WP-15): every attempt that
    returned a response (known usage), every attempt that raised before its
    response was read (unknown usage, never a zero), and the optional repair
    call. Each row carries its attempt record, and the spec's result carries
    them all (``ReviewResult.call_usage``). Workers run on pool threads and
    must not touch ``log`` / ``diagnostics`` directly; the coordinator applies
...
def _telemetry_row(
    ...
    *,
    ...
    """One call's ``record_api_call`` kwargs, carrying its attempt record.

    The attempt record is the row's billing input (plan WP-15); the flat
    fields are its display summary. An attempt that raised before its
    response was read has unknown usage — the row says so rather than
    recording zero tokens as if they were measured.
    """
...
def _response_attempt(result: ReviewResult, *, model: str, role: str) -> AttemptUsage:
    """A call that returned a response: its usage is the response's."""
...
def _raised_attempt(*, model: str, role: str) -> AttemptUsage:
    """A call that raised before its response was read: usage unknown."""
...
def _review_one_spec(
    ...
    raised before its response was read (unknown usage). The returned result
    carries all of them on ``call_usage``, so a repaired spec keeps the
    truncated call it replaced.
    """
    ...
    def _record(result: ReviewResult, attempt: AttemptUsage, *, retry_status: str, level: str | None = None) -> None:
    ...
    def _outcome(result: ReviewResult) -> _SpecReviewOutcome:
        ...
        try:
            ...
            if result.parse_status in REPAIRABLE_PARSE_STATUSES:
                ...
                try:
                    ...
                    _record(
                        repair_result,
                        _response_attempt(repair_result, model=model, role=ROLE_REPAIR),
                        retry_status="retry",
                    )
                    ...
                    # The repair was sent; its usage was never read.
                    _record(
                        ReviewResult(
                            findings=[],
                            model=model,
                            error=f"Real-time review repair failed: {repair_exc}",
                        ),
                        _raised_attempt(model=model, role=ROLE_REPAIR),
                        retry_status="retry",
                    )
                return _outcome(result)
            ...
            # One row per call that raised, whether or not it is retried:
            # the request was sent and its usage was never read.
            _record(
                failed,
                _raised_attempt(model=model, role=role),
                retry_status=retry_status,
                level="error" if terminal else "warning",
            )
            if terminal:
