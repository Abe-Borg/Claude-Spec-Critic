...
|---|---|---|---|---|---|---|
| **M1** | Remove the resume / durable-state subsystem | 🟠 (Directed) | **No** | Large | — | ✅ **Done** — this branch |
| **M2** | Resolve the orphaned cross-check suppression feature | 🟠 High | For now | Small (delete) / Med (wire) | needs your decision | — |
| **M3** | Purge the auto-apply / locator fossils | 🟡 Medium | Yes | Medium | — | ✅ **Done** — PR #198 (merged) |
...
| **M6** | Fix CLAUDE.md / README staleness | 🟡 Medium | For now | Small | M1, M2, M3 (doc tail) | 🟡 Steps 1,2,5 done; step 3 awaits M2 |
...
- **M1 — Remove the resume / durable-state subsystem** — ✅ **done on this branch** (with the M6 doc tail, steps 2 & 5). **Step 1 (preserve the emit dependency):** relocated `serialize_edit_proposal` out of `resume_state.py` into `edit_sidecar.py` as a self-contained `_serialize_edit_proposal` (no private helpers needed); the `.edits.json` sidecar is unchanged on disk. **Steps 2–5:** deleted `src/orchestration/resume_state.py` + `src/batch/batch_state_store.py`; gutted `batch_controller.py` to a forward-only flow (removed the 4 state-store proxies, the `PHASE_*` / `build_resume_state` / `SUPPORTED_PHASES` imports, all 7 `save_batch_state(build_resume_state(...))` phase-boundary calls + the now-dead `verification_completed` bookkeeping var, `check_pending_batch` + the pending-batch dialog, `format_batch_age`, `is_valid_verification_resume_state`, `resume_batch`, `resume_verification_poll`, `resume_cross_check_verification_poll`; reworded the poll-failure message to drop "Resume later" and the cross-check-skip warning to drop "resumable"); `review_run_controller.py` lost the `delete_batch_state` import + call and the orphaned `_reattach_recorder` re-export; `gui.py` lost the resume imports, the startup `self.after(500, self._check_pending_batch)` hook, and the 6 resume delegating methods; `pipeline.py` needed no change (no resume usage). Also swept the now-dead `app_paths` batch-state symbols (`BATCH_STATE_FILENAME`, the two age constants, `batch_state_path`, `app_state_dir`, the `user_state_dir` import) and reworded 3 stale `resume_state` doc-comments (`reviewer.py`, `capture_hooks.py`, `session.py`). **Step 6 (tests):** removed resume round-trip coverage only — stripped the dedicated `Resume state round-trip` class from 11 chunk/telemetry/m/o files (kept every constructor + verification-**cache** round-trip + feature test) and the 3 resume tests + orphaned `_minimal_submission` helper from `test_tracing.py`; dropped the imports left unused by the strip. **M6 doc tail:** deleted CLAUDE.md §9 (renumbered §10→§9, §11→§10), scrubbed the source-layout tree + 6 Chunk invariant paragraphs (telemetry now round-trips through the verification cache only) and the README (batch-processing bullet, contested/budget telemetry, and the deleted "Batch-resume continuity" section). **Two deliberate scoping calls:** (1) **kept** `tracing.session.reattach_run_recorder` and its direct `test_reattach_recorder_*` tests — generic tracing infra with its own coverage, now production-unused after resume removal; left as a candidate follow-up sweep (same pattern as the M3 fixture note). (2) **kept** the live `CollectedBatchState.cross_check_skipped_due_to_missing_specs` field/branch (pipeline-set state; only dropped the "resumable" wording). **Verification:** hermetic suite green (**628 passed**, down from 677 — exactly the 49 deleted resume round-trip tests); pyflakes clean across every edited `src/` and `tests/` file (no undefined names, no unused imports). The GUI can't be imported/launched in the sandbox (no `tkinter`), so the forward-only batch flow and the removed pending-batch prompt are verified by AST-parse + pyflakes + reference-grep + logic review only — a live end-to-end batch run remains an unverified gap (same limitation noted for M3).
