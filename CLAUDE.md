...
│   ├── digest.py               # one sheet → one Opus 4.8 vision request → structured text (sanitized errors + transient retry)
│   ├── digest_cache.py         # persistent, content-keyed cache of per-sheet digests (skips re-paying for vision)
│   ├── pipeline.py             # orchestration: PDFs → sheets → (bounded-parallel) digests → optional synthesis → combined text (DrawingContext)
