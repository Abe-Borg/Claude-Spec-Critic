...
### Module registry (module-extraction refactor, Phase 1)
`src/modules/` introduces `ReviewModule` — one frozen object per reviewable domain, carrying identity (`module_id`, display strings) plus the `CodeCycle`. The registry (`AVAILABLE_MODULES` / `DEFAULT_MODULE` / `get_module`) mirrors the `AVAILABLE_CYCLES` pattern: unknown/missing ids degrade to the default `california_k12_mep` module, and `validate_module_registry` fails at import on duplicate module ids or duplicate cycle labels (**cycle labels are registry-unique — they namespace the verification cache**). Orchestration entry points (`start_batch_review`, `reconstruct_batch_submission`, `thin_submission_from_batch_results`, `start_batch_verification`, `collect_batch_verification_results`) take `module=` instead of `cycle=`; stage functions with access to a submission derive the module from `submission.module_id` (`run_cross_check_for_batch`, the review-repair path, `run_batch_collection_headless`) so one state object can't pair one module's identity with another's cycle. `module_id` persists on `BatchSubmission` / `PipelineResult` / `PendingBatch` (additive field, **no pending-batch schema bump** — legacy state files load with the default module) and lands in trace `run.json`. Content layers (prompts, cross-checker, verifier, preprocessor) still take `cycle=` — they gain module slots in later phases. The Phase 0 pins (`tests/test_golden_domain_surfaces.py` goldens + `tests/test_domain_routing_pins.py`) must stay byte-green through every extraction phase; `tests/test_module_registry.py` locks the registry + round-trip contracts.
