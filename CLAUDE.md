...
| `standard_reasoning` | default for substantive technical claims | Sonnet | on | severity-based | opt-in (3 fetches; default off) | yes |
| `deep_reasoning` | escalated, OR initial pass for CRITICAL `california_ahj` | Opus | on | severity-based | opt-in (3 fetches; default off) | no (terminal) |
...
| `SPEC_CRITIC_ENABLE_WEB_FETCH` | off | Enable to attach the Chunk 11 `web_fetch` server tool (and its `web-fetch-2026-02-09` beta header) to STANDARD_REASONING / DEEP_REASONING verification requests. Default **off** because an account without the beta enabled rejects the header with HTTP 400 `invalid_request_error` (an unrecognized `anthropic-beta` value is NOT silently ignored), which crashes every verification request routed to a fetch-eligible mode — the common path. When off, the tool is never appended so the header is never attached and verification falls back to web_search only (pre-Chunk-11 behavior). Flip on once the beta is available on the account. |
