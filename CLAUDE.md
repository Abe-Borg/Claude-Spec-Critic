...
### Module registry (module-extraction refactor, Phases 1-2)
`src/modules/` introduces `ReviewModule` — one frozen object per reviewable domain, carrying identity (`module_id`, display strings), the `CodeCycle`, and (Phase 2) the **prompt content slots**: `reviewer_persona`, `review_user_intro`, `review_severity_definitions`, `review_confidence_high_example`, `review_categories_template` (formatted against `category_format_kwargs(cycle)` placeholders), `review_examples`, `cross_check_persona`, `cross_check_severity_definitions`, `verifier_persona`, `verifier_source_priorities`. The registry (`AVAILABLE_MODULES` / `DEFAULT_MODULE` / `get_module`) mirrors the `AVAILABLE_CYCLES` pattern: unknown/missing ids degrade to the default `california_k12_mep` module, and `validate_module_registry` fails at import on duplicate module ids, duplicate cycle labels (**cycle labels are registry-unique — they namespace the verification cache AND back the `module_for_cycle` bridge**), empty prompt slots, a categories template that doesn't format against the module's own cycle, or few-shot examples that violate the real parse contract (every JSON example is run through `reviewer.validate_edit_shape`; examples must not mention `evidenceElementId` / element-id tags — the block sits inside the cached system-prompt prefix).
