...
│   ├── drawing_export.py       # Pure (tkinter-free) serialization of a drawing digest to a folder of .md files
...
│   ├── digest.py               # one sheet → one Opus 4.8 vision request → structured text (shared request-shape builder; sanitized errors + transient retry)
│   ├── file_upload.py          # Files-API transport: upload a sheet's images → file_id content blocks (keeps each request body under the 32 MB limit)
│   ├── batch_digest.py         # batch-mode digest: render-stream → Files-API upload → one Message Batch → poll → collect (50% cheaper)
│   ├── digest_cache.py         # persistent, content-keyed cache of per-sheet digests (skips re-paying for vision)
│   ├── pipeline.py             # orchestration: PDFs → sheets → (bounded-parallel OR batched) digests → optional synthesis → combined text (DrawingContext)
│   ├── synthesis.py            # cross-sheet reconciliation: per-sheet digests → one set-level overview (text-only Opus 4.8 call)
│   ├── cost.py                 # pre-run cost estimate for a drawing set (feeds the cost-confirm dialog)
...
### Project Context attachments & drawing digests
Project Context is free-text that ships on **every** review, cross-check, AND verification call, so anything spliced into it makes all phases aware of that content at plain-text cost. **File attachments** feed it — `.docx` / `.pdf` (text extracted) and `.md` / `.txt` (read verbatim) via `input/extractor.extract_context_text`, wrapped + merged + cap-checked by the pure (tkinter-free, unit-testable) helpers in `gui/context_attachment.py` (stable delimiter shape + the hard `PROJECT_CONTEXT_MAX_TOKENS` (100k) cap; an over-cap merge is **refused, never truncated**). **Drawing digests do NOT feed Project Context** — analyzing drawings and feeding the review are deliberately decoupled. The "Analyze Drawings…" button runs the `src/drawings` vision engine (`extract_drawing_context`, by default `use_batch=True` → one Message Batch of one Opus 4.8 vision request per sheet, images via the Files API → a `DrawingContext` carrying per-sheet `SheetDigest`s plus an optional cross-sheet synthesis; see "Drawing digest: batch mode + Files-API transport" below) on a **worker thread** (the digest is slow — minutes for a large set), then (`gui/context_controller._apply_drawing_result`) asks the operator for a folder and **writes the whole result to disk** via the pure `gui/drawing_export.py`: one `.md` per sheet (failed sheets included, each carrying its error), `00_synthesis.md`, `combined.md`, and an `00_index.md` run summary (counts + per-sheet status table), under a timestamped `<stem>_drawings_<ts>/` subfolder; Project Context is left untouched. The save dialog is skipped (with a warning) when `ok_sheet_count == 0` — nothing was read, so there is nothing to save. The drawing engine is imported **lazily** (single seam `_run_drawing_extraction`) so the controller stays importable without PyMuPDF and tests can inject a fake `DrawingContext`. Per-sheet API failures are sanitized — `digest._clean_error` maps an upstream 5xx HTML page to a one-liner (e.g. `502 bad gateway (server temporarily unavailable — try again)`) so a cloudflare error page never reaches the dialog — and *transient* failures (5xx / 429 / connection / timeout, per `digest._is_transient_error`) are retried with exponential backoff (`DEFAULT_DIGEST_MAX_RETRIES`, injectable `sleep`) on top of the SDK's own retries. The digest is plain Markdown saved to disk; the operator reviews the files and decides separately what (if anything) to attach to a review as a `.md` file attachment (the file-attach path above), so a drawing run never silently inflates a review's token bill, and there is nothing drawing-related to persist for review-batch resume. The standalone analyzer `python -m src.drawings` saves the combined digest as a single `.md`. Per-sheet digests are content-cached (`drawings/digest_cache.py`, persistent JSON keyed on the rendered images + model + prompt fingerprint + output params), so re-running a set after editing one sheet only re-pays vision for the changed sheet (`SheetDigest.cached` / `DrawingContext.cached_sheet_count` report the hits); caching is **opt-in** via `extract_drawing_context(use_cache=True)` — the GUI/standalone paths pass it, hermetic tests don't, so the on-disk file is never touched in tests. In the **real-time path** (`use_batch=False` — the standalone library path and any non-GUI caller), per-sheet digests are **dispatched concurrently** on a bounded thread pool (`DEFAULT_DIGEST_WORKERS=4`; override via `max_workers=` or `SPEC_CRITIC_DRAWING_MAX_WORKERS`, `1` = sequential) while **rendering stays sequential** on the calling thread (PyMuPDF is not thread-safe to share); results are reassembled in page order so the combined digest and every token total are deterministic regardless of completion order, at most `workers` rendered sheets are held in flight (bounding memory), and the thread-safe `DigestCache` lock keeps a concurrent re-run safe. With `synthesize=True` (the GUI/standalone pass it) one extra **text-only** pass (`synthesis.synthesize_drawing_set`, Opus 4.8 by default, override via `SPEC_CRITIC_DRAWING_SYNTHESIS_MODEL`) reconciles the per-sheet digests into a "Drawing Set Overview" (cross-sheet tag references + conflicts) prepended to `combined_text` and exposed as `DrawingContext.synthesis_text`; it is **skipped for <2 readable sheets** and **falls back to the plain per-sheet digests on failure** (the failure recorded in `errors`, never losing the run), and it reuses `digest.py`'s sanitize + transient-retry. Locked in by `tests/test_drawing_export.py` (folder layout, per-sheet incl. failed-sheet files, synthesis fallback, index contents, collision-safe write; hermetic), `tests/test_context_attachments.py` (file-attachment extraction + merge/cap helpers, hermetic), `tests/test_drawing_context_integration.py` (threaded analyze→save flow leaves Project Context untouched; skips without customtkinter), `tests/test_drawing_digest_cache.py` (key sensitivity + store/persistence + digest_sheet integration, hermetic), `tests/test_drawing_digest.py` (a barrier-based concurrency proof + page-order preservation), and `tests/test_drawing_synthesis.py` (skip/​success/​exclude-failed/​clean-error/​retry + pipeline prepend/fallback).

### Drawing cost-confirm gate
Reading drawings is the app's most expensive action (one Opus 4.8 vision call per sheet, each carrying ~37 images), so both run paths surface an estimate and require confirmation before any vision call. `core/pricing.py` is the dependency-free pricing source — USD-per-MTok rates (Opus 4.8 `$5`/`$25`, Sonnet 4.6 `$3`/`$15`, Haiku 4.5 `$1`/`$5`; **image/vision tokens bill as ordinary input tokens**; `estimate_request_cost(..., batch=True)` applies the 50% Batch discount). **Rates drift — verify against published pricing; unknown model ids return `None`** so the dialog shows scale without inventing a dollar figure. `price_for` tolerates dated/`-fast` suffixes via a longest-prefix fallback. `drawings/cost.estimate_drawing_set_cost(sheet_count, …)` turns a sheet count into a `DrawingCostEstimate` (image tokens via `estimate_image_tokens_for_set` + fixed per-sheet prompt/output assumptions + the optional synthesis pass when ≥2 sheets) — a deliberately rough, slightly-high heads-up, **not** an invoice; `format_drawing_cost_prompt` renders the dialog text. **Layering:** `core` stays dependency-free; the drawing-specific estimate lives in the drawings layer (`drawings`→`core`), never the reverse. The main app gates in `gui/context_controller.attach_drawings` via the monkeypatchable seams `_estimate_drawing_cost` (counts sheets with `render.list_sheets`, priced at the **Batch rate** via `estimate_drawing_set_cost(..., batch=True)`; returns `None` on any failure) → `_confirm_drawing_cost`. The confirm is the **explicit "Analyze" gate and is ALWAYS shown** — a `None` estimate still gates behind a generic confirmation, so selecting drawings never silently fires a batch (the earlier "skip the prompt when the estimate is `None`" behavior was the gap that let API calls start on upload). The standalone analyzer gates in `_on_process` and also renders the (batch-rate) estimate inline in its summary. Locked in by `tests/test_pricing.py` (rates, batch discount + batch-prompt text, suffix resolution, token math, single-sheet/​unknown-model handling) and `tests/test_drawing_context_integration.py` (gate always shown, incl. on a `None` estimate).

### Drawing digest: batch mode + Files-API transport (the 32 MB fix)
The GUI ("Analyze Drawings…") and the standalone analyzer both digest drawings through the **Message Batches API** by default — `extract_drawing_context(use_batch=True)` (`gui/context_controller._run_drawing_extraction`, `drawings/gui.py`). This was driven by a hard failure: the real-time path inlined a sheet's overview + 36 tiles = **37 base64 PNGs in one `messages.create`**, and a dense E-size sheet's payload exceeds Anthropic's **32 MB Messages-API request-size limit**, which the API rejects with **HTTP 400** — so *every* sheet in a permit set failed at once (the progress callback fires on completion regardless of success, so the GUI log said "Analyzed page N" for sheets that had actually 400'd, and the all-failed dialog wrongly blamed "a temporary issue"). Hermetic tests never caught it because the fake client never serializes the payload — the same failure class as the retired `web-fetch` beta header.

**The fix is the Files API** (`drawings/file_upload.py`): upload each image once and reference it by `file_id`, so the request body is a handful of ids (well under 32 MB, and under the 256 MB batch envelope). `batch_digest.py` orchestrates render-stream → cache-or-upload → one `client.beta.messages.batches.create(requests, betas=["files-api-2025-04-14"])` → bounded poll (`client.messages.batches.retrieve`, beta-create / non-beta-poll mirrors the review path) → collect (`client.messages.batches.results`) → parse → write cache → **delete the uploaded files** (best-effort; *left in place on a detach* so a still-running remote batch keeps its inputs). Invariants preserved: the digest request shape is the single `digest.build_digest_request_params` (shared with real-time `digest_sheet`, so model/`thinking:adaptive`/`effort:high` can't drift); the digest **cache** short-circuits a sheet before any upload or batch item (unchanged sheet on a re-run costs nothing); results are assembled in **page order** regardless of completion order; per-sheet failures (upload error, item `errored`/`expired`, missing result) are captured on that sheet's `SheetDigest` and never abort the set. Cross-sheet **synthesis stays one synchronous text-only call** after the batch (no images → no size issue). The real-time base64 path (`digest_sheet`, `use_batch=False`) remains a tested library function. `digest._clean_error` now **keeps the API message on a 4xx** (`HTTP 400: ...request too large...`) instead of collapsing it to a bare `HTTP 400` — that collapse is exactly what hid this failure. **Known v1 gap:** a drawing batch that exceeds the 4 h poll bound detaches without resume (the review batch's `batch_resume.py` is not wired for drawings); the per-sheet error names the still-running batch id. Locked in by `tests/test_drawing_batch.py` (Files-API upload uses file_ids not base64; submit/collect happy-path + request shape; page-order independence; errored / missing item; cache hit skips upload+batch; partial-cache submits only the miss; upload-failure capture; detach leaves files + marks sheets; pipeline `use_batch=True` end-to-end).

### Drawing render dimension cap (the 2000 px many-image fix)
The next real-API-only failure surfaced *after* the Files-API fix above unblocked submission: every sheet in a permit set failed with `invalid_request_error: ...image.source...data: At least one of the image dimensions exceed max allowed size for many-image requests: 2000 pixels`. Per Anthropic's vision docs, the per-image hard cap is 8000×8000 px **but drops to 2000×2000 px once a request carries >20 images, and an image over the cap is *rejected* (HTTP 400), not downscaled** (the <=20-image case silently resizes; >20 does not). Every drawing-digest request is 1 overview + 36 tiles = **37 images**, so the 2000 px hard cap always applies. The error's `...source...base64.data` path is just how the API labels the *decoded* image — it fires identically whether the bytes ride inline or via a Files-API `file_id`, which is why the 32 MB fix didn't prevent it.

Root cause: `tiling` targeted the long edge at **exactly 2000 px**, but PyMuPDF sizes a clipped pixmap by rounding the transformed clip to a whole-pixel rectangle (floor low corner, ceil high corner — MuPDF `fz_round_rect`), so any interior tile whose origin (`cell*r - overlap`) doesn't land on a pixel boundary after scaling renders at **2001 px** and trips the rejection (empirically **30 of 36 tiles** overshoot on an E-size sheet → whole request rejected → all sheets fail). The fix renders a margin **under** the cap: `tiling.MANY_IMAGES_LONG_EDGE_CAP_PX` (2000) is the hard ceiling, `TARGET_LONG_EDGE_PX_MANY_IMAGES = cap - _MANY_IMAGES_RENDER_MARGIN_PX` (8 px ⇒ 1992) is the render target — proven overshoot is ≤1 px/axis, 8 px is generous headroom at <0.5% linear-resolution cost (worst rendered edge becomes 1993 px). `TARGET_LONG_EDGE_PX_FEW_IMAGES` (2576) is unchanged: the <=20-image hard cap is 8000 px and an oversized image is downscaled there, so an off-by-one is harmless and no margin is needed. `estimate_image_tokens_for_set` reads the same `target_long_edge_px`, so the cost estimate tracks what's actually rendered. Hermetic tests missed the original bug because a fake client never rasterizes/serializes a pixmap (same class as the 32 MB and `web-fetch` failures); `tests/test_drawing_tiling.py` now locks it in **without PyMuPDF** by simulating `fz_round_rect` over the 6×6 grid across several aspect ratios and asserting no rendered edge exceeds the cap (this test fails at the old 2000 px target).
...
| `SPEC_CRITIC_STRICT_TOOL_USE` | off | Enable with any truthy value to attach `strict: true` to the review / cross-check / verification / triage tool schemas (grammar-constrains tool input to the schema). Default **off** because the strict-mode × adaptive-thinking interaction is unverified from the hermetic harness; a wrong default would 400 every submit. The schemas are already strict-compatible — smoke-test against the live API, then enable. Rollback with `0` / `false` / `no` / `off`. |
...
| `SPEC_CRITIC_DRAWING_CACHE_PERSIST` | on | Disable to keep the drawing-digest cache in-memory only (no disk file) |
| `SPEC_CRITIC_DRAWING_CACHE_PATH` | `~/.spec_critic/drawing_digest_cache.json` | Override the on-disk drawing-digest cache path; `~` and `$VAR` are expanded |
| `SPEC_CRITIC_DRAWING_MAX_WORKERS` | `4` | Per-set digest concurrency (threads); `1` forces sequential. Per-call `max_workers=` overrides it; malformed values fall back to 4 |
| `SPEC_CRITIC_DRAWING_SYNTHESIS_MODEL` | Opus 4.8 | Override the cross-sheet synthesis model (the text-only set-overview pass) |
