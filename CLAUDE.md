...
**Continuation-cap parity (batch ↔ real-time).** The batch wave loop's pause-turn check `if continuation_counts[key] > cap` uses `>` *deliberately* — it mirrors the real-time loop's `range(max_continuations + 1)` budget (1 initial call + `cap` resume rounds). The counter is incremented before the check, so `> cap` allows the initial wave plus exactly `cap` follow-up resubmissions before terminal-UNVERIFIED. `MAX_VERIFICATION_WAVES` (3) == `DEFAULT_MAX_CONTINUATIONS` (2) + 1, so the two caps coincide on the final wave. Do **not** "tighten" this to `>=`: it would terminate one resume early and make batch verification stricter than real-time. Locked by `tests/test_batch_continuation_cap.py`. (STRUCTURAL_AUDIT P2-1.)
