...
| Orchestration & dedup ([**Ch 7**](07_orchestration.md)) | `test_dedup_edit_identity`, `test_edit_occurrences`, `test_batch_escalation` | `_deduplicate_findings` preserves every member in `occurrence_originals`; a display group keeps one occurrence per file and validated place, so the same fix at p4 and p8 is two occurrences, a duplicate emission is one, and members with no usable element are one uncertain occurrence; a missing original borrows no locator; occurrence ids are content-derived and module-qualified; the sidecar writes one entry per occurrence (schemas 6 / 7), pinned by a golden beside the legacy one it replaced; real-time and batch escalation share merge/disagreement logic and preserve VERIFIED_CONTESTED. |
| Applier: sidecars and conflicts ([**Ch 7**](07_orchestration.md)) | `test_applier_occurrence_schemas`, `test_applier_conflicts`, `test_occurrence_end_to_end` | The applier reads schemas 4 and 5 exactly as before and the occurrence-aware 6 and 7 by their contract (a required `occurrence_id` and `location_basis`, a unique `(module_id, occurrence_id)` key), and refuses any other; every edit is planned against the unmutated document, overlapping instructions are all held as `EDIT_CONFLICT`, identical ones are written once, and no order of the sidecar changes the document or any outcome. End to end, on real documents, the report, the sidecar, the applier, and the receipt name every executable place alike: the same fix at p4 and p8 is two tracked changes, two files with two places each are four. |
...
| Verification grounding & cache ([**Ch 10**](10_verification_grounding.md)) | `test_source_grounding_invariant`, `test_source_quote_schema`, `test_cache_visibility`, `test_verification_cache_serialization`, `test_verdict_classification_contract`, `test_verification_cache_eligibility` | Supportive verdicts without an accepted citation downgrade; the same scripted response classifies identically on both transports, and a malformed or missing verdict is an operational failure that keeps its usage; one cache predicate at write, read, and load reuses only grounded conclusive verdicts (never an UNVERIFIED), ignoring legacy and invalid rows one by one; cache TTL defaults to 60 days; the persisted/skipped field split covers every `VerificationResult` field. |
...
| Config, capability & tokens ([**Ch 12**](12_configuration_and_models.md)) | `test_capability_policy`, `test_token_budgets` *(marked)*, `test_request_budget`, `test_pinned_standards_editions`, `test_env_flag_overrides`, `test_budget_exhaustion` | Haiku/triage phases never carry a `thinking` key; per-phase output caps clamp to the model ceiling; every large request is sized for the model that runs it — an API count is an estimate, a malformed one is never a zero, the padded fallback covers every part of the request, an oversized package is split and a spec that cannot fit is named, never truncated; pinned editions reach the prompts; env flags override defaults; budget exhaustion is a sub-label, never a new status. |
| Location-aware review & compliance ([**Ch 19**](19_location_aware_review.md)) | `test_requirements_research`, `test_compliance_pass`, `test_compliance_completeness`, `test_datacenter_e2e` | Only grounded specification requirements are controlling; every one gets a coverage row, and one no request assessed is NOT ASSESSED, never missing or represented; `missing` needs every chunk, so a failed or skipped chunk or an excluded spec holds an addition as report-only instead of emitting it; the report, sidecar, profile export, diagnostics, and program roll-up all say when coverage is incomplete; a profile-less run is byte-identical. |
...
| HTML report & Ask AI ([**Ch 22**](22_html_report.md)) | `test_html_report_exporter`, `test_html_report_javascript`, `test_html_chat_behavior` | The HTML report says what the Word report says and ships one CSP-hashed script that parses; run under Node against scripted streams, the chat adds a question to the conversation only when the model finishes answering it, so no failure can leave a tool call without its result in the next request, and the API key stays in page memory. |
