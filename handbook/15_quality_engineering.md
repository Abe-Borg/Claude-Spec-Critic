...
| Orchestration & dedup ([**Ch 7**](07_orchestration.md)) | `test_dedup_edit_identity`, `test_edit_occurrences`, `test_batch_escalation` | `_deduplicate_findings` preserves every member in `occurrence_originals`; a display group keeps one occurrence per file and validated place, so the same fix at p4 and p8 is two occurrences, a duplicate emission is one, and members with no usable element are one uncertain occurrence; a missing original borrows no locator; occurrence ids are content-derived and module-qualified; the schema 4 / 5 sidecar is byte-identical to master until S12; real-time and batch escalation share merge/disagreement logic and preserve VERIFIED_CONTESTED. |
| Applier: sidecars and conflicts ([**Ch 7**](07_orchestration.md)) | `test_applier_occurrence_schemas`, `test_applier_conflicts` | The applier reads schemas 4 and 5 exactly as before and the occurrence-aware 6 and 7 by their contract (a required `occurrence_id` and `location_basis`, a unique `(module_id, occurrence_id)` key), and refuses any other; every edit is planned against the unmutated document, overlapping instructions are all held as `EDIT_CONFLICT`, identical ones are written once, and no order of the sidecar changes the document or any outcome. |
