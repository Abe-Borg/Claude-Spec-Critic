...
# ---------------------------------------------------------------------------
# 7.5 Evidence model & grounded invariant
# ---------------------------------------------------------------------------


def test_verification_result_has_phase3_fields():
    ...
    assert r.cache_status == "n/a"
    assert r.web_search_requests == 0
    assert r.successful_source_count == 0
    assert r.search_error_count == 0


def test_grounded_invariant_downgrades_ungrounded_confirmed():
    from src.verifier import VerificationResult, _enforce_grounding_invariant
    r = VerificationResult(verdict="CONFIRMED", grounded=False, explanation="Looks fine")
    out = _enforce_grounding_invariant(r)
    assert out.verdict == "UNVERIFIED"
    assert "downgraded" in out.explanation.lower() or "external" in out.explanation.lower()


def test_grounded_invariant_keeps_grounded_confirmed():
    from src.verifier import VerificationResult, _enforce_grounding_invariant
    r = VerificationResult(verdict="CONFIRMED", grounded=True, sources=["https://example.com"])
    out = _enforce_grounding_invariant(r)
    assert out.verdict == "CONFIRMED"


def test_grounded_invariant_does_not_touch_unverified():
    from src.verifier import VerificationResult, _enforce_grounding_invariant
    r = VerificationResult(verdict="UNVERIFIED", grounded=False, explanation="No search")
    out = _enforce_grounding_invariant(r)
    assert out.verdict == "UNVERIFIED"


def test_local_skip_result_is_unverified_and_marked():
...
# ---------------------------------------------------------------------------
# 7.2 VerificationCache
# ---------------------------------------------------------------------------


def test_verification_cache_hits_share_results():
    from src.verifier import VerificationResult
    from src.verification_cache import VerificationCache

    cache = VerificationCache()
    f1 = _make_finding()
    f2 = _make_finding()  # identical claim → should hit
    grounded_result = VerificationResult(
        verdict="CONFIRMED", grounded=True, sources=["https://dgs.ca.gov"], model_used="claude-opus-4-6",
    )
    cache.put(f1, cycle=DEFAULT_CYCLE, result=grounded_result)
    hit = cache.get(f2, cycle=DEFAULT_CYCLE)
...
def test_verification_cache_distinguishes_different_claims():
    """Two findings citing the same code reference but different claims must verify separately."""
    from src.verifier import VerificationResult
    from src.verification_cache import VerificationCache, make_cache_key

    cache = VerificationCache()
    f1 = _make_finding(issue="CBC 2025 edition is current", existing="per CBC 2019")
...
def test_verification_cache_does_not_cache_ungrounded_results():
    from src.verifier import VerificationResult
    from src.verification_cache import VerificationCache
    cache = VerificationCache()
    f = _make_finding()
    cache.put(f, cycle=DEFAULT_CYCLE, result=VerificationResult(verdict="UNVERIFIED", grounded=False))
