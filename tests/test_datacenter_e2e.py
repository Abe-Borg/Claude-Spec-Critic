"""WS-5 end-to-end: the whole location-aware pipeline, hermetically.

Drives ``run_batch_collection_headless`` for the ``datacenter_fire`` module
...
*artifacts* a WS-5 run must produce:

- the report's "Jurisdiction & Client Requirements" section, the project /
  client title lines, the coverage matrix + advisories + edition table, the
  ``[Compliance]``-labeled findings, and the two Run-Diagnostics banner rows;
- the sidecar (schema v4) with the ``project`` block, the
  ``requirements_coverage`` matrix, and a compliance edit entry (``lc-`` id);
- the standalone ``<report-stem>.profile.json`` export;
- verification round 2 actually receiving the compliance findings.

And it proves CA-neutrality: the SAME driver, run for the (profile-less)
California module through the identical code path, produces none of those
WS-4/WS-5 surfaces and never hands a compliance finding to verification
(invariant 2). All client/location identifiers are anonymized fixtures.
"""
...
from src.modules import DATACENTER_FIRE, DEFAULT_MODULE
...
# ---------------------------------------------------------------------------
# Fixtures — a two-spec data-center fire package + a researched profile.
# ---------------------------------------------------------------------------

_WET = "21 13 13 - Wet-Pipe Sprinkler Systems.docx"
_ALARM = "28 31 00 - Fire Detection and Alarm.docx"

_WET_CONTENT = (
    "PART 1 - GENERAL\n\n"
    "1.01 SUMMARY\n\n"
    "A. Comply with 2015 IBC Chapter 9.\n\n"
    "B. Provide wet-pipe sprinkler systems per NFPA 13."
)
_ALARM_CONTENT = "1.01 SUMMARY\n\nA. Provide fire detection and alarm as indicated."


def _specs() -> list[ExtractedSpec]:
    return [
        ExtractedSpec(filename=_WET, content=_WET_CONTENT, word_count=20),
        ExtractedSpec(filename=_ALARM, content=_ALARM_CONTENT, word_count=9),
    ]
...
def _requirements_profile() -> RequirementsProfile:
    ...
    items = [
        ...
        ResearchItem(
            ...
            requirement="The adopted code references NFPA 13-2019.",
            authority="Virginia DHCD",
        ...
        ResearchItem(
            ...
            requirement="ExampleCo prefers double-interlock pre-action in white space.",
            authority="ExampleCo",
            grounded=False,
            confidence=0.4,
        ),
...
def _review_finding() -> Finding:
    # EDIT whose existingText is verbatim in _WET_CONTENT so it survives the
...
def _cross_finding() -> Finding:
    return Finding(
        ...
        fileName=_ALARM,
        section="3.05",
        issue="Division 28 detection zoning does not match the Division 21 pre-action zones.",
...
def _compliance_result() -> ReviewResult:
    # An ADD anchored verbatim on _WET_CONTENT (survives anchor validation) so
    # a compliance edit lands in the sidecar; coverage exercises the report's
    # represented / missing rows.
    add = Finding(
        severity="HIGH",
        fileName=_WET,
        section="1.01",
        issue="The package does not reference the adopted NFPA 13-2019 edition "
        "(profile requirement r-000000000002).",
        actionType="ADD",
        existingText=None,
        replacementText="C. Comply with NFPA 13-2019 as referenced by the adopted code.",
        codeReference="IBC 903",
    ...
    return ReviewResult(
        ...
        coverage=[
            {"requirement_id": "r-000000000001", "status": "represented",
             "evidence": "Comply with 2015 IBC Chapter 9.", "fileName": _WET},
...
def _submission(module, *, profile, requirements_profile, cross_check) -> BatchSubmission:
    cid0, cid1 = "review__21_13_13__0", "review__28_31_00__1"
    job = BatchJob(
        batch_id="msgbatch_DC_E2E",
        job_type="review",
        request_map={
            cid0: {"filename": _WET, "index": 0, "type": "review"},
            cid1: {"filename": _ALARM, "index": 1, "type": "review"},
        },
        created_at=1_700_000_000.0,
    )
    return BatchSubmission(
        job=job,
        files_reviewed=[_WET, _ALARM],
...
# ---------------------------------------------------------------------------
# The data-center (flag ON) end-to-end run.
# ---------------------------------------------------------------------------


class TestDatacenterEndToEnd:
    def _run(self, monkeypatch):
        verify_spy = _VerificationSpy()
        review_map = {
            "review__21_13_13__0": ReviewResult(
                findings=[_review_finding()], parse_status="ok"
            ),
            "review__28_31_00__1": ReviewResult(findings=[], parse_status="ok"),
        }
        _install_fakes(
            monkeypatch,
    ...
    def test_pipeline_result_carries_all_phases(self, monkeypatch):
        ...
        assert result.module_id == "datacenter_fire"
    ...
    def test_report_has_ws5_surfaces(self, monkeypatch, tmp_path):
        ...
        # Coverage matrix (represented + missing), advisories, edition table.
    ...
    def test_sidecar_v4_and_profile_json(self, monkeypatch, tmp_path):
        ...
        assert profile_json["module_id"] == "datacenter_fire"
        assert profile_json["requirements_profile"]["items"]


# ---------------------------------------------------------------------------
# CA-neutrality: the same driver, profile-less California module.
# ---------------------------------------------------------------------------


class TestCaliforniaNeutralityThroughSamePath:
    def _run(self, monkeypatch):
        verify_spy = _VerificationSpy()
        review_map = {
            "review__21_13_13__0": ReviewResult(
        ...
        _install_fakes(
            ...
            verify_spy=verify_spy,
            compliance_result=None,  # never patched — CA compliance is a no-op
        )
        submission = _submission(
            DEFAULT_MODULE,
            profile=None,
    ...
    def test_no_ws4_ws5_surfaces_and_no_compliance(self, monkeypatch, tmp_path):
        result, verify_spy = self._run(monkeypatch)

        # Compliance never ran; no profile rides the result.
        assert result.compliance_result is None
        assert result.project_profile is None
        assert result.requirements_profile is None

        out = tmp_path / "ca_report.docx"
        export_report(result, out)
        text = _doc_text(out)
        ...
        # No compliance finding was ever handed to verification.
        assert not any(
            (f.section or "").startswith("[Compliance]") for f in verify_spy.all_findings
        )

        # Sidecar degrades to the profile-less shape (still schema v4, but no
        # project block and an empty coverage matrix).
        sidecar_path = write_edit_instructions_sidecar(result, out)
        sidecar = json.loads(sidecar_path.read_text())
        assert sidecar["project"] is None
        assert sidecar["requirements_coverage"] == []
        assert write_requirements_profile_sidecar(result, out) is None
